...
//   const likedProducts = useSelector((state) => state.amazon.likedProducts);
//   const loading = useSelector((state) => state.amazon.loading);
...
//               <div key={index}>
//                 <img className="w-16 h-16" src={product?.likedProduct?.image} />
//                 <p className="text-black">{product?.likedProduct?.title}</p>
//                 <p>{product?.likedProduct?.category_name}</p>
//                 <p>{product?.likedProduct?.description}</p>
//                 <p>${product?.likedProduct?.price}</p>
//                 <Button
//                   className="w-[300px] py-1.5 text-md font-normal rounded-sm bg-gradient-to-t from-[#f7dfa5] to-[#f0c15b] hover:bg-gradient-to-b border border-zinc-400 active:border-yellow-800 active:shadow-amazonInput"
      ...
      <div className="flex flex-col jusitfy-start items-start">
        <div className="mt-3 text-center w-full">
          <h1 className="text-3xl lg:text-4xl tracking-tight font-semibold leading-8 lg:leading-9 text-gray-800 dark:text-white">
            Wish List <FavoriteIcon className="text-red-600" />
          </h1>
        </div>
        <div className="mt-4">
