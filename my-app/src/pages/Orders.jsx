...
const Orders = () => {
  ...
  const purchasedItems = useSelector((state) => state.orders.purchasedItems);
  ...
  const handleDeletePurchasedItem = (itemId) => {
    dispatch(deletePurchasedItems(itemId));
  };

  return (
    <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto dark:bg-amazonBlue">
        ...
        {Array.isArray(purchasedItems) && purchasedItems.length > 0 ? (
          purchasedItems.map((item) => (
            <div key={item.id}>
              ...
              <button onClick={() => handleDeletePurchasedItem(item.id)}>
                Delete
              </button>
            </div>
          ))
        ) : (
          <p className="dark:text-white">No purchased items found.</p>
        )}
